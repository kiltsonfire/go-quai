...
type subscription struct {
	...
	headers   chan *types.WorkObjects
...
func (es *EventSystem) subscribeMinedPendingLogs(crit quai.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		...
		headers:   make(chan *types.WorkObjects),
...
func (es *EventSystem) SubscribeNewHeads(headers chan *types.WorkObjects) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) handleChainEvent(filters filterIndex, ev core.ChainEvent) {
	for _, f := range filters[BlocksSubscription] {
		f.headers <- &types.WorkObjects{ev.Block}
	}
}
