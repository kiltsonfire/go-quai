...
func (p *Peer) RequestHeadersByNumber(origin uint64, amount int, to uint64, dom bool, reverse bool) error {
	p.Log().Debug("Fetching batch of headers", "count", amount, "fromnum", origin, "to", to, "dom", dom, "reverse", reverse)
	query := GetBlockHeadersPacket{
