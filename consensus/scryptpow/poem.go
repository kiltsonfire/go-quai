...
func (scryptpow *Scryptpow) CalcOrder(chain consensus.BlockReader, header *types.WorkObject) (*big.Int, int, error) {
	...
	err := scryptpow.verifySeal(header.WorkObjectHeader())
	if err != nil {
		return big.NewInt(0), -1, err
	}

	// Get entropy reduction of this header using scrypt hash
	powHash, err := scryptpow.ComputePowHash(header.WorkObjectHeader())
	if err != nil {
		return big.NewInt(0), -1, err
	}
	intrinsicEntropy = scryptpow.IntrinsicLogEntropy(powHash)
	target := new(big.Int).Div(common.Big2e256, header.Difficulty())
	zoneThresholdEntropy := scryptpow.IntrinsicLogEntropy(common.BytesToHash(target.Bytes()))
...
func (scryptpow *Scryptpow) IntrinsicLogEntropy(powHash common.Hash) *big.Int {
...
func (scryptpow *Scryptpow) TotalLogEntropy(chain consensus.ChainHeaderReader, header *types.WorkObject) *big.Int {
	// Treating the genesis block differntly
	if chain.IsGenesisHash(header.Hash()) {
		return big.NewInt(0)
	}
	intrinsicS, order, err := scryptpow.CalcOrder(chain, header)
	if err != nil {
		scryptpow.logger.WithFields(log.Fields{"err": err, "hash": header.Hash()}).Error("Error calculating order in TotalLogEntropy")
		return big.NewInt(0)
	}
	if scryptpow.NodeLocation().Context() == common.ZONE_CTX {
		workShareS, err := scryptpow.WorkShareLogEntropy(chain, header)
		if err != nil {
			scryptpow.logger.WithFields(log.Fields{"err": err, "hash": header.Hash()}).Error("Error calculating WorkShareLogEntropy in TotalLogEntropy")
			return big.NewInt(0)
		}
...
func (scryptpow *Scryptpow) DeltaLogEntropy(chain consensus.ChainHeaderReader, header *types.WorkObject) *big.Int {
	// Treating the genesis block differntly
	if chain.IsGenesisHash(header.Hash()) {
		return big.NewInt(0)
	}
	intrinsicS, order, err := scryptpow.CalcOrder(chain, header)
	if err != nil {
		scryptpow.logger.WithFields(log.Fields{"err": err, "hash": header.Hash()}).Error("Error calculating order in DeltaLogEntropy")
		return big.NewInt(0)
	}
	if scryptpow.NodeLocation().Context() == common.ZONE_CTX {
		workShareS, err := scryptpow.WorkShareLogEntropy(chain, header)
		if err != nil {
			scryptpow.logger.WithFields(log.Fields{"err": err, "hash": header.Hash()}).Error("Error calculating WorkShareLogEntropy in DeltaLogEntropy")
			return big.NewInt(0)
		}
		intrinsicS = new(big.Int).Add(intrinsicS, workShareS)
	}
...
func (scryptpow *Scryptpow) UncledLogEntropy(block *types.WorkObject) *big.Int {
	uncles := block.Uncles()
	totalUncledLogS := big.NewInt(0)
	for _, uncle := range uncles {
		// Verify the seal and get the powHash for the given header
		err := scryptpow.verifySeal(uncle)
		if err != nil {
			continue
		}
		// Get entropy reduction of this header using scrypt hash
		powHash, err := scryptpow.ComputePowHash(uncle)
		if err != nil {
			continue
		}
		intrinsicEntropy := scryptpow.IntrinsicLogEntropy(powHash)
...
func (scryptpow *Scryptpow) WorkShareLogEntropy(chain consensus.ChainHeaderReader, wo *types.WorkObject) (*big.Int, error) {
	workShares := wo.Uncles()
	totalWsEntropy := big.NewInt(0)
	for _, ws := range workShares {
		powHash, err := scryptpow.ComputePowHash(ws)
		if err != nil {
		...
		if new(big.Int).SetBytes(powHash.Bytes()).Cmp(target) > 0 { // powHash > target
			...
			cBigBits := scryptpow.IntrinsicLogEntropy(powHash)
			thresholdBigBits := scryptpow.IntrinsicLogEntropy(common.BytesToHash(target.Bytes()))
...
func (scryptpow *Scryptpow) UncledDeltaLogEntropy(chain consensus.ChainHeaderReader, header *types.WorkObject) *big.Int {
	// Treating the genesis block differntly
	if chain.IsGenesisHash(header.Hash()) {
		return big.NewInt(0)
	}
	...
	if err != nil {
		scryptpow.logger.WithField("err", err).Error("Error calculating order in UncledDeltaLogEntropy")
		return big.NewInt(0)
	}
...
func (scryptpow *Scryptpow) CalcRank(chain consensus.ChainHeaderReader, header *types.WorkObject) (int, error) {
	if chain.IsGenesisHash(header.Hash()) {
		return 0, nil
	}
	_, order, err := scryptpow.CalcOrder(chain, header)
...
func (scryptpow *Scryptpow) CheckIfValidWorkShare(workShare *types.WorkObjectHeader) types.WorkShareValidity {
	thresholdDiff := params.WorkSharesThresholdDiff
	if scryptpow.CheckWorkThreshold(workShare, thresholdDiff) {
		return types.Valid
	} else if scryptpow.CheckWorkThreshold(workShare, scryptpow.config.WorkShareThreshold) {
		return types.Sub
	} else {
...
func (scryptpow *Scryptpow) CheckWorkThreshold(workShare *types.WorkObjectHeader, workShareThresholdDiff int) bool {
	workShareMinTarget, err := consensus.CalcWorkShareThreshold(workShare, workShareThresholdDiff)
	if err != nil {
		return false
	}
	powHash, err := scryptpow.ComputePowHash(workShare)
	if err != nil {
		return false
	}
