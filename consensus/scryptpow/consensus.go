...
// Scryptpow proof-of-work protocol constants.
var (
...
func (scryptpow *Scryptpow) Author(header *types.WorkObject) (common.Address, error) {
	return header.PrimaryCoinbase(), nil
}

// VerifyHeader checks whether a header conforms to the consensus rules of the
// stock Quai scryptpow engine.
func (scryptpow *Scryptpow) VerifyHeader(chain consensus.ChainHeaderReader, header *types.WorkObject) error {
	// If we're running a full engine faking, accept any input as valid
	if scryptpow.config.PowMode == ModeFullFake {
		return nil
	}
	nodeCtx := scryptpow.config.NodeLocation.Context()
	...
	return scryptpow.verifyHeader(chain, header, parent, false, time.Now().Unix())
}

// VerifyHeaders is similar to VerifyHeader, but verifies a batch of headers
// concurrently. The method returns a quit channel to abort the operations and
// a results channel to retrieve the async verifications.
func (scryptpow *Scryptpow) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.WorkObject) (chan<- struct{}, <-chan error) {
	// If we're running a full engine faking, accept any input as valid
	if scryptpow.config.PowMode == ModeFullFake || len(headers) == 0 {
	...
	for i := 0; i < workers; i++ {
		go func() {
			...
			for index := range inputs {
				errors[index] = scryptpow.verifyHeaderWorker(chain, headers, index, unixNow)
				done <- index
			}
		}()
	}
...
func (scryptpow *Scryptpow) verifyHeaderWorker(chain consensus.ChainHeaderReader, headers []*types.WorkObject, index int, unixNow int64) error {
	nodeCtx := scryptpow.config.NodeLocation.Context()
	...
	return scryptpow.verifyHeader(chain, headers[index], parent, false, unixNow)
}

// VerifyUncles verifies that the given block's uncles conform to the consensus
// rules of the stock Quai scryptpow engine.
func (scryptpow *Scryptpow) VerifyUncles(chain consensus.ChainReader, block *types.WorkObject) error {
	nodeCtx := scryptpow.config.NodeLocation.Context()
	// If we're running a full engine faking, accept any input as valid
	if scryptpow.config.PowMode == ModeFullFake {
		return nil
	...
	for _, uncle := range block.Uncles() {
		...
		if nodeCtx == common.ZONE_CTX {
			...
			expected := scryptpow.CalcDifficulty(chain, parent.WorkObjectHeader(), parent.ExpansionNumber())
...
func (scryptpow *Scryptpow) verifyHeader(chain consensus.ChainHeaderReader, header, parent *types.WorkObject, uncle bool, unixNow int64) error {
	nodeCtx := scryptpow.config.NodeLocation.Context()
	...
	_, order, err := scryptpow.CalcOrder(chain, header)
	if err != nil {
		return err
	}
	if order > nodeCtx {
	...
	if !scryptpow.config.NodeLocation.InSameSliceAs(header.Location()) {
	...
	parentEntropy := scryptpow.TotalLogEntropy(chain, parent)
	...
	if nodeCtx > common.PRIME_CTX {
		_, parentOrder, _ := scryptpow.CalcOrder(chain, parent)
		...
		if parentOrder < nodeCtx {
			...
			parentDeltaEntropy := scryptpow.DeltaLogEntropy(chain, parent)
	...
	if nodeCtx > common.PRIME_CTX {
		...
		if parentOrder < nodeCtx {
			...
			expectedParentUncledDeltaEntropy := scryptpow.UncledDeltaLogEntropy(chain, parent)
	...
	if nodeCtx == common.ZONE_CTX {
		...
		if err != nil {
			return fmt.Errorf("out-of-scope primary coinbase in the header: %v location: %v nodeLocation: %v, err %s", header.PrimaryCoinbase(), header.Location(), scryptpow.config.NodeLocation, err)
		}
		...
		expectedGasLimit := core.CalcGasLimit(parent, scryptpow.config.GasCeil)
...
func (scryptpow *Scryptpow) CalcDifficulty(chain consensus.ChainHeaderReader, parent *types.WorkObjectHeader, expansionNum uint8) *big.Int {
	nodeCtx := scryptpow.config.NodeLocation.Context()

	if nodeCtx != common.ZONE_CTX {
		scryptpow.logger.WithField("context", nodeCtx).Error("Cannot CalcDifficulty for non-zone context")
		return nil
	}
	...
	if chain.IsGenesisHash(parent.Hash()) {
		...
		genesisTotalLogEntropy := scryptpow.TotalLogEntropy(chain, genesis)
		if genesisTotalLogEntropy.Cmp(genesis.ParentEntropy(common.PRIME_CTX)) < 0 { // prevent negative difficulty
			scryptpow.logger.Errorf("Genesis block has invalid parent entropy: %v", genesis.ParentEntropy(common.PRIME_CTX))
			return nil
		}
	...
	if x.Cmp(scryptpow.config.MinDifficulty) < 0 {
		x.Set(scryptpow.config.MinDifficulty)
	}
	return x
}

func (scryptpow *Scryptpow) IsDomCoincident(chain consensus.ChainHeaderReader, header *types.WorkObject) bool {
	_, order, err := scryptpow.CalcOrder(chain, header)
	if err != nil {
		scryptpow.logger.Error("Error calculating calc order, err ", err)
		return false
	}
	return order < scryptpow.config.NodeLocation.Context()
}

// VerifySeal returns the PowHash and the verifySeal output
func (scryptpow *Scryptpow) VerifySeal(header *types.WorkObjectHeader) (common.Hash, error) {
	return header.Hash(), scryptpow.verifySeal(header)
}

// verifySeal checks whether a block satisfies the PoW difficulty requirements,
// either using the usual scryptpow cache for it, or alternatively using a full DAG
// to make remote mining fast.
func (scryptpow *Scryptpow) verifySeal(header *types.WorkObjectHeader) error {
	// If we're running a fake PoW, accept any seal as valid
	if scryptpow.config.PowMode == ModeFake || scryptpow.config.PowMode == ModeFullFake {
		time.Sleep(scryptpow.fakeDelay)
		if scryptpow.fakeFail == header.NumberU64() {
...
// header to conform to the scryptpow protocol. The changes are done inline.
func (scryptpow *Scryptpow) Prepare(chain consensus.ChainHeaderReader, header *types.WorkObject, parent *types.WorkObject) error {
	header.WorkObjectHeader().SetDifficulty(scryptpow.CalcDifficulty(chain, parent.WorkObjectHeader(), parent.ExpansionNumber()))
	return nil
}
...
func (scryptpow *Scryptpow) Finalize(chain consensus.ChainHeaderReader, batch ethdb.Batch, header *types.WorkObject, state *state.StateDB, setRoots bool, utxoSetSize uint64, utxosCreate, utxosDelete []common.Hash, supplyRemovedQi *big.Int) (*multiset.MultiSet, uint64, []*types.SpentUtxoEntry, error) {
	nodeLocation := scryptpow.config.NodeLocation
	nodeCtx := scryptpow.config.NodeLocation.Context()

	if nodeLocation.Equal(common.Location{0, 0}) {
		err := state.AddLockedBalances(header.Number(common.ZONE_CTX), scryptpow.config.GenAllocs, scryptpow.logger)
		if err != nil {
	...
	for denomination, depth := range types.TrimDepths {
		if denomination <= types.MaxTrimDenomination && header.NumberU64(nodeCtx) > depth {
			...
			go func(denomination uint8, depth uint64) {
				...
				TrimBlock(chain, batch, denomination, header.NumberU64(nodeCtx)-depth, nextBlockToTrim, &utxosDelete, &trimmedUtxos, supplyRemovedQi, &utxoSetSize, !setRoots, &lock, scryptpow.logger) // setRoots is false when we are processing the block
				wg.Done()
			}(denomination, depth)
		}
	}
	...
	if len(trimmedUtxos) > 0 {
		scryptpow.logger.Infof("Trimmed %d UTXOs from db in %s", len(trimmedUtxos), common.PrettyDuration(time.Since(start)))
	}
	if !setRoots {
...
func (scryptpow *Scryptpow) NodeLocation() common.Location {
	return scryptpow.config.NodeLocation
}

func (scryptpow *Scryptpow) ComputePowLight(header *types.WorkObjectHeader) (common.Hash, common.Hash) {
	panic("compute pow light doesnt exist for scrypt")
}

// ComputePowHash computes the PoW hash using scrypt algorithm
func (scryptpow *Scryptpow) ComputePowHash(header *types.WorkObjectHeader) (common.Hash, error) {
	// Create input data for scrypt
	data := header.SealHash().Bytes()
	
	// Use scrypt parameters: N=32768, r=1, p=1, keyLen=32
	// These are reasonable parameters for PoW (similar to Litecoin)
	hash, err := scrypt.Key(data, data[:8], 32768, 1, 1, 32)
	if err != nil {
