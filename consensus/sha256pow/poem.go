...
func (sha256pow *Sha256pow) CalcOrder(chain consensus.BlockReader, header *types.WorkObject) (*big.Int, int, error) {
	...
	err := sha256pow.verifySeal(header.WorkObjectHeader())
	if err != nil {
		return big.NewInt(0), -1, err
	}

	// Get entropy reduction of this header using SHA256 hash
	powHash, err := sha256pow.ComputePowHash(header.WorkObjectHeader())
	if err != nil {
		return big.NewInt(0), -1, err
	}
	intrinsicEntropy = sha256pow.IntrinsicLogEntropy(powHash)
	target := new(big.Int).Div(common.Big2e256, header.Difficulty())
	zoneThresholdEntropy := sha256pow.IntrinsicLogEntropy(common.BytesToHash(target.Bytes()))
...
func (sha256pow *Sha256pow) IntrinsicLogEntropy(powHash common.Hash) *big.Int {
...
func (sha256pow *Sha256pow) TotalLogEntropy(chain consensus.ChainHeaderReader, header *types.WorkObject) *big.Int {
	// Treating the genesis block differntly
	if chain.IsGenesisHash(header.Hash()) {
		return big.NewInt(0)
	}
	intrinsicS, order, err := sha256pow.CalcOrder(chain, header)
	if err != nil {
		sha256pow.logger.WithFields(log.Fields{"err": err, "hash": header.Hash()}).Error("Error calculating order in TotalLogEntropy")
		return big.NewInt(0)
	}
	if sha256pow.NodeLocation().Context() == common.ZONE_CTX {
		workShareS, err := sha256pow.WorkShareLogEntropy(chain, header)
		if err != nil {
			sha256pow.logger.WithFields(log.Fields{"err": err, "hash": header.Hash()}).Error("Error calculating WorkShareLogEntropy in TotalLogEntropy")
			return big.NewInt(0)
		}
...
func (sha256pow *Sha256pow) DeltaLogEntropy(chain consensus.ChainHeaderReader, header *types.WorkObject) *big.Int {
	// Treating the genesis block differntly
	if chain.IsGenesisHash(header.Hash()) {
		return big.NewInt(0)
	}
	intrinsicS, order, err := sha256pow.CalcOrder(chain, header)
	if err != nil {
		sha256pow.logger.WithFields(log.Fields{"err": err, "hash": header.Hash()}).Error("Error calculating order in DeltaLogEntropy")
		return big.NewInt(0)
	}
	if sha256pow.NodeLocation().Context() == common.ZONE_CTX {
		workShareS, err := sha256pow.WorkShareLogEntropy(chain, header)
		if err != nil {
			sha256pow.logger.WithFields(log.Fields{"err": err, "hash": header.Hash()}).Error("Error calculating WorkShareLogEntropy in DeltaLogEntropy")
			return big.NewInt(0)
		}
		intrinsicS = new(big.Int).Add(intrinsicS, workShareS)
	}
...
func (sha256pow *Sha256pow) UncledLogEntropy(block *types.WorkObject) *big.Int {
	uncles := block.Uncles()
	totalUncledLogS := big.NewInt(0)
	for _, uncle := range uncles {
		// Verify the seal and get the powHash for the given header
		err := sha256pow.verifySeal(uncle)
		if err != nil {
			continue
		}
		// Get entropy reduction of this header using SHA256 hash
		powHash, err := sha256pow.ComputePowHash(uncle)
		if err != nil {
			continue
		}
		intrinsicEntropy := sha256pow.IntrinsicLogEntropy(powHash)
...
func (sha256pow *Sha256pow) WorkShareLogEntropy(chain consensus.ChainHeaderReader, wo *types.WorkObject) (*big.Int, error) {
	workShares := wo.Uncles()
	totalWsEntropy := big.NewInt(0)
	for _, ws := range workShares {
		powHash, err := sha256pow.ComputePowHash(ws)
		if err != nil {
		...
		if new(big.Int).SetBytes(powHash.Bytes()).Cmp(target) > 0 { // powHash > target
			...
			cBigBits := sha256pow.IntrinsicLogEntropy(powHash)
			thresholdBigBits := sha256pow.IntrinsicLogEntropy(common.BytesToHash(target.Bytes()))
...
func (sha256pow *Sha256pow) UncledDeltaLogEntropy(chain consensus.ChainHeaderReader, header *types.WorkObject) *big.Int {
	// Treating the genesis block differntly
	if chain.IsGenesisHash(header.Hash()) {
		return big.NewInt(0)
	}
	...
	if err != nil {
		sha256pow.logger.WithField("err", err).Error("Error calculating order in UncledDeltaLogEntropy")
		return big.NewInt(0)
	}
...
func (sha256pow *Sha256pow) CalcRank(chain consensus.ChainHeaderReader, header *types.WorkObject) (int, error) {
	if chain.IsGenesisHash(header.Hash()) {
		return 0, nil
	}
	_, order, err := sha256pow.CalcOrder(chain, header)
...
func (sha256pow *Sha256pow) CheckIfValidWorkShare(workShare *types.WorkObjectHeader) types.WorkShareValidity {
	thresholdDiff := params.WorkSharesThresholdDiff
	if sha256pow.CheckWorkThreshold(workShare, thresholdDiff) {
		return types.Valid
	} else if sha256pow.CheckWorkThreshold(workShare, sha256pow.config.WorkShareThreshold) {
		return types.Sub
	} else {
...
func (sha256pow *Sha256pow) CheckWorkThreshold(workShare *types.WorkObjectHeader, workShareThresholdDiff int) bool {
	workShareMinTarget, err := consensus.CalcWorkShareThreshold(workShare, workShareThresholdDiff)
	if err != nil {
		return false
	}
	powHash, err := sha256pow.ComputePowHash(workShare)
	if err != nil {
		return false
	}
