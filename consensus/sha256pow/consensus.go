...
// Sha256pow proof-of-work protocol constants.
var (
...
func (sha256pow *Sha256pow) Author(header *types.WorkObject) (common.Address, error) {
	return header.PrimaryCoinbase(), nil
}

// VerifyHeader checks whether a header conforms to the consensus rules of the
// stock Quai sha256pow engine.
func (sha256pow *Sha256pow) VerifyHeader(chain consensus.ChainHeaderReader, header *types.WorkObject) error {
	// If we're running a full engine faking, accept any input as valid
	if sha256pow.config.PowMode == ModeFullFake {
		return nil
	}
	nodeCtx := sha256pow.config.NodeLocation.Context()
	...
	return sha256pow.verifyHeader(chain, header, parent, false, time.Now().Unix())
}

// VerifyHeaders is similar to VerifyHeader, but verifies a batch of headers
// concurrently. The method returns a quit channel to abort the operations and
// a results channel to retrieve the async verifications.
func (sha256pow *Sha256pow) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.WorkObject) (chan<- struct{}, <-chan error) {
	// If we're running a full engine faking, accept any input as valid
	if sha256pow.config.PowMode == ModeFullFake || len(headers) == 0 {
	...
	for i := 0; i < workers; i++ {
		go func() {
			...
			for index := range inputs {
				errors[index] = sha256pow.verifyHeaderWorker(chain, headers, index, unixNow)
				done <- index
			}
		}()
	}
...
func (sha256pow *Sha256pow) verifyHeaderWorker(chain consensus.ChainHeaderReader, headers []*types.WorkObject, index int, unixNow int64) error {
	nodeCtx := sha256pow.config.NodeLocation.Context()
	...
	return sha256pow.verifyHeader(chain, headers[index], parent, false, unixNow)
}

// VerifyUncles verifies that the given block's uncles conform to the consensus
// rules of the stock Quai sha256pow engine.
func (sha256pow *Sha256pow) VerifyUncles(chain consensus.ChainReader, block *types.WorkObject) error {
	nodeCtx := sha256pow.config.NodeLocation.Context()
	// If we're running a full engine faking, accept any input as valid
	if sha256pow.config.PowMode == ModeFullFake {
		return nil
	...
	for _, uncle := range block.Uncles() {
		...
		if nodeCtx == common.ZONE_CTX {
			...
			expected := sha256pow.CalcDifficulty(chain, parent.WorkObjectHeader(), parent.ExpansionNumber())
...
func (sha256pow *Sha256pow) verifyHeader(chain consensus.ChainHeaderReader, header, parent *types.WorkObject, uncle bool, unixNow int64) error {
	nodeCtx := sha256pow.config.NodeLocation.Context()
	...
	_, order, err := sha256pow.CalcOrder(chain, header)
	if err != nil {
		return err
	}
	if order > nodeCtx {
	...
	if !sha256pow.config.NodeLocation.InSameSliceAs(header.Location()) {
	...
	parentEntropy := sha256pow.TotalLogEntropy(chain, parent)
	...
	if nodeCtx > common.PRIME_CTX {
		_, parentOrder, _ := sha256pow.CalcOrder(chain, parent)
		...
		if parentOrder < nodeCtx {
			...
			parentDeltaEntropy := sha256pow.DeltaLogEntropy(chain, parent)
	...
	if nodeCtx > common.PRIME_CTX {
		...
		if parentOrder < nodeCtx {
			...
			expectedParentUncledDeltaEntropy := sha256pow.UncledDeltaLogEntropy(chain, parent)
	...
	if nodeCtx == common.ZONE_CTX {
		...
		if err != nil {
			return fmt.Errorf("out-of-scope primary coinbase in the header: %v location: %v nodeLocation: %v, err %s", header.PrimaryCoinbase(), header.Location(), sha256pow.config.NodeLocation, err)
		}
		...
		expectedGasLimit := core.CalcGasLimit(parent, sha256pow.config.GasCeil)
...
func (sha256pow *Sha256pow) CalcDifficulty(chain consensus.ChainHeaderReader, parent *types.WorkObjectHeader, expansionNum uint8) *big.Int {
	nodeCtx := sha256pow.config.NodeLocation.Context()

	if nodeCtx != common.ZONE_CTX {
		sha256pow.logger.WithField("context", nodeCtx).Error("Cannot CalcDifficulty for non-zone context")
		return nil
	}
	...
	if chain.IsGenesisHash(parent.Hash()) {
		...
		genesisTotalLogEntropy := sha256pow.TotalLogEntropy(chain, genesis)
		if genesisTotalLogEntropy.Cmp(genesis.ParentEntropy(common.PRIME_CTX)) < 0 { // prevent negative difficulty
			sha256pow.logger.Errorf("Genesis block has invalid parent entropy: %v", genesis.ParentEntropy(common.PRIME_CTX))
			return nil
		}
	...
	if x.Cmp(sha256pow.config.MinDifficulty) < 0 {
		x.Set(sha256pow.config.MinDifficulty)
	}
	return x
}

func (sha256pow *Sha256pow) IsDomCoincident(chain consensus.ChainHeaderReader, header *types.WorkObject) bool {
	_, order, err := sha256pow.CalcOrder(chain, header)
	if err != nil {
		sha256pow.logger.Error("Error calculating calc order, err ", err)
		return false
	}
	return order < sha256pow.config.NodeLocation.Context()
}

// VerifySeal returns the PowHash and the verifySeal output
func (sha256pow *Sha256pow) VerifySeal(header *types.WorkObjectHeader) (common.Hash, error) {
	return header.Hash(), sha256pow.verifySeal(header)
}

// verifySeal checks whether a block satisfies the PoW difficulty requirements,
// either using the usual sha256pow cache for it, or alternatively using a full DAG
// to make remote mining fast.
func (sha256pow *Sha256pow) verifySeal(header *types.WorkObjectHeader) error {
	// If we're running a fake PoW, accept any seal as valid
	if sha256pow.config.PowMode == ModeFake || sha256pow.config.PowMode == ModeFullFake {
		time.Sleep(sha256pow.fakeDelay)
		if sha256pow.fakeFail == header.NumberU64() {
...
// header to conform to the sha256pow protocol. The changes are done inline.
func (sha256pow *Sha256pow) Prepare(chain consensus.ChainHeaderReader, header *types.WorkObject, parent *types.WorkObject) error {
	header.WorkObjectHeader().SetDifficulty(sha256pow.CalcDifficulty(chain, parent.WorkObjectHeader(), parent.ExpansionNumber()))
	return nil
}
...
func (sha256pow *Sha256pow) Finalize(chain consensus.ChainHeaderReader, batch ethdb.Batch, header *types.WorkObject, state *state.StateDB, setRoots bool, utxoSetSize uint64, utxosCreate, utxosDelete []common.Hash, supplyRemovedQi *big.Int) (*multiset.MultiSet, uint64, []*types.SpentUtxoEntry, error) {
	nodeLocation := sha256pow.config.NodeLocation
	nodeCtx := sha256pow.config.NodeLocation.Context()

	if nodeLocation.Equal(common.Location{0, 0}) {
		err := state.AddLockedBalances(header.Number(common.ZONE_CTX), sha256pow.config.GenAllocs, sha256pow.logger)
		if err != nil {
	...
	for denomination, depth := range types.TrimDepths {
		if denomination <= types.MaxTrimDenomination && header.NumberU64(nodeCtx) > depth {
			...
			go func(denomination uint8, depth uint64) {
				...
				TrimBlock(chain, batch, denomination, header.NumberU64(nodeCtx)-depth, nextBlockToTrim, &utxosDelete, &trimmedUtxos, supplyRemovedQi, &utxoSetSize, !setRoots, &lock, sha256pow.logger) // setRoots is false when we are processing the block
				wg.Done()
			}(denomination, depth)
		}
	}
	...
	if len(trimmedUtxos) > 0 {
		sha256pow.logger.Infof("Trimmed %d UTXOs from db in %s", len(trimmedUtxos), common.PrettyDuration(time.Since(start)))
	}
	if !setRoots {
...
func (sha256pow *Sha256pow) NodeLocation() common.Location {
	return sha256pow.config.NodeLocation
}

func (sha256pow *Sha256pow) ComputePowLight(header *types.WorkObjectHeader) (common.Hash, common.Hash) {
	panic("compute pow light doesnt exist for sha256")
}

// ComputePowHash computes the PoW hash using SHA256 algorithm
func (sha256pow *Sha256pow) ComputePowHash(header *types.WorkObjectHeader) (common.Hash, error) {
	// Create input data for SHA256
	data := header.SealHash().Bytes()
	
	// Use double SHA256 (similar to Bitcoin)
