...
// ReadDeltaS retrieves a block's change in entropy corresponding to the hash.
func ReadSubDeltaS(db ethdb.Reader, hash common.Hash, number uint64) []*big.Float {
	data, _ := db.Get(headerSubDeltaSKey(number, hash))
	if len(data) == 0 {
		return nil
	}
	deltaS := new([][]byte)
...
// WriteDeltaS stores the change in entropy of a block into the database.
func WriteSubDeltaS(db ethdb.KeyValueWriter, hash common.Hash, number uint64, deltaS []*big.Float) {
	...
	if err := db.Put(headerSubDeltaSKey(number, hash), encodedData); err != nil {
		log.Crit("Failed to store block total entropy", "err", err)
	}
}

// DeleteSubDeltaS removes all block change in entropy data associated with a hash.
func DeleteSubDeltaS(db ethdb.KeyValueWriter, hash common.Hash, number uint64) {
	if err := db.Delete(headerSubDeltaSKey(number, hash)); err != nil {
