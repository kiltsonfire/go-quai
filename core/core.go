...
func (c *Core) InsertChain(blocks types.Blocks) (int, error) {
	...
	for idx, block := range blocks {
		...
		if !c.sl.engine.IsDomCoincident(block.Header()) {
			newPendingEtxs, _, err := c.sl.Append(block.Header(), types.EmptyHeader(), common.Hash{}, big.NewFloat(0), false, true, nil)
			if err == nil {
...
func (c *Core) Append(header *types.Header, domPendingHeader *types.Header, domTerminus common.Hash, domS *big.Float, domOrigin bool, reorg bool, newInboundEtxs types.Transactions) ([]types.Transactions, *big.Float, error) {
	newPendingEtxs, s, err := c.sl.Append(header, domPendingHeader, domTerminus, domS, domOrigin, reorg, newInboundEtxs)
	if err != nil {
