...
func (c *Core) EntropyWindow() *big.Int {
	var err error
	powhash, exists := c.sl.hc.powHashCache.Peek(c.CurrentHeader().Hash())
	if !exists {
		powhash, err = c.engine.VerifySeal(c.CurrentHeader().WorkObjectHeader())
		if err != nil {
			return nil
		}
		c.sl.hc.powHashCache.Add(c.CurrentHeader().Hash(), powhash)
	}
	currentBlockIntrinsic := c.engine.IntrinsicLogEntropy(powhash)
	MaxAllowableEntropyDist := new(big.Int).Mul(currentBlockIntrinsic, big.NewInt(c_maxAllowableEntropyDist))
