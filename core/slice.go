...
func (sl *Slice) generateSlicePendingHeader(block *types.WorkObject, newTermini types.Termini, domPendingHeader *types.WorkObject, domOrigin bool, subReorg bool, fill bool) (types.PendingHeader, error) {
	...
	pendingHeaderWithTermini.WorkObject().SetLocation(block.Header().Location())
...
func (sl *Slice) init(genesis *Genesis) error {
	...
	if sl.hc.Empty() {
		...
		err := sl.hc.AddPendingEtxs(types.PendingEtxs{Header: genesisHeader, Etxs: emptyPendingEtxs})
		if err != nil {
			return err
		}
		err = sl.AddPendingEtxsRollup(types.PendingEtxsRollup{Header: genesisHeader, EtxsRollup: emptyPendingEtxs})
		if err != nil {
			return err
		}
