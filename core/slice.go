...
type Slice struct {
	...
	bestPhKeyObject types.BestPhKeyObject
...
func (sl *Slice) relayPh(pendingHeaderWithTermini types.PendingHeader, domOrigin bool, location common.Location) {
	...
	if nodeCtx == common.ZONE_CTX {
		bestPh, exists := sl.phCache[sl.bestPhKeyObject.Key()]
		if exists {
...
func (sl *Slice) GetPendingHeader() (*types.Header, error) {
	if ph := sl.phCache[sl.bestPhKeyObject.Key()].Header; ph != nil {
		return ph, nil
	} else {
...
func (sl *Slice) writeToPhCacheAndPickPhHead(pendingHeaderWithTermini types.PendingHeader) {
	...
	if sl.poem(newPhEntropy, sl.bestPhKeyObject.Entropy()) {
		sl.bestPhKeyObject = types.NewBestPhKey(pendingHeaderWithTermini.Termini[c_terminusIndex], newPhEntropy, pendingHeaderWithTermini.Header.ParentHash())
...
func (sl *Slice) init(genesis *Genesis) error {
	...
	if sl.hc.Empty() {
		...
		sl.bestPhKeyObject = types.NewBestPhKey(genesisHash, big.NewInt(0), genesisHash)
...
func (sl *Slice) loadLastState() error {
	...
	sl.bestPhKeyObject = rawdb.ReadBestPhKeyObject(sl.sliceDb)
...
func (sl *Slice) Stop() {
	...
	rawdb.WriteBestPhKeyObject(sl.sliceDb, sl.bestPhKeyObject)
